...
// Validate checks the field values on Configs_Config with the rules defined in
// the proto definition for this message. If any rules are violated, the first
// error encountered is returned, or nil if there are no violations.
func (m *Configs_Config) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on Configs_Config with the rules defined
// in the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in Configs_ConfigMultiError,
// or nil if none found.
func (m *Configs_Config) ValidateAll() error {
	return m.validate(true)
}

func (m *Configs_Config) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	if utf8.RuneCountInString(m.GetField()) < 1 {
		err := Configs_ConfigValidationError{
			field:  "Field",
			reason: "value length must be at least 1 runes",
		}
		if !all {
	...
	if len(errors) > 0 {
		return Configs_ConfigMultiError(errors)
	}

	return nil
}

// Configs_ConfigMultiError is an error wrapping multiple validation errors
// returned by Configs_Config.ValidateAll() if the designated constraints
// aren't met.
type Configs_ConfigMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m Configs_ConfigMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m Configs_ConfigMultiError) AllErrors() []error { return m }

// Configs_ConfigValidationError is the validation error returned by
// Configs_Config.Validate if the designated constraints aren't met.
type Configs_ConfigValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e Configs_ConfigValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e Configs_ConfigValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e Configs_ConfigValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e Configs_ConfigValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e Configs_ConfigValidationError) ErrorName() string { return "Configs_ConfigValidationError" }

// Error satisfies the builtin error interface
func (e Configs_ConfigValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
...
var _ error = Configs_ConfigValidationError{}
